...
<section class="ds-section">
			...
			>
				{#each basicTags as tag, i}
					<span style="
						...
						font-size: 12px; font-weight: 500; color: var(--dt2); white-space: nowrap;
					">
						{tag}
						<button
							onclick={() => basicRemove(i)}
							style="display: flex; align-items: center; justify-content: center; width: 14px; height: 14px; border: none; background: var(--dbg2); border-radius: 50%; cursor: pointer; padding: 0; color: var(--dt3); flex-shrink: 0;"
				...
				<input
					...
					style="
						...
						font-size: 13px; color: var(--dt2); outline: none; padding: 2px 0;
					"
					aria-label="New tag"
				/>
			</div>
			<div style="margin-top: 6px; font-size: 11px; color: var(--dt4);">
		...
		<div style="max-width: 480px; position: relative;">
			...
			>
				{#each suggestTags as tag, i}
					...
					">
						...
						<button
							...
							style="display: flex; align-items: center; justify-content: center; width: 14px; height: 14px; border: none; background: var(--dbg2); border-radius: 50%; cursor: pointer; padding: 0; color: var(--dt3);"
				...
				<input
					...
					style="flex: 1; min-width: 100px; border: none; background: transparent; font-size: 13px; color: var(--dt2); outline: none; padding: 2px 0;"
			...
			{#if showDropdown && filteredSuggestions.length > 0}
				<div style="
					...
					background: var(--dbg3); border: 1px solid var(--dbd); border-radius: 8px;
					...
					{#each filteredSuggestions as suggestion}
						<button
							...
							style="
								...
								font-size: 13px; color: var(--dt3); cursor: pointer;
	...
	<div class="ds-card">
		...
		<div style="max-width: 480px;">
			<div style="
				...
				padding: 8px 10px; border-radius: 8px;
				background: var(--dbg);
				border: 1px solid {maxTags.length >= MAX_TAGS ? 'var(--dbd)' : 'var(--dbd)'};
			...
			>
				{#each maxTags as tag, i}
					<span style="
						...
						padding: 3px 8px 3px 10px; border-radius: 9999px;
						background: var(--dbg2); border: 1px solid var(--dbd);
						font-size: 12px; font-weight: 500; color: var(--dt2);
					">
						{tag}
						<button
							onclick={() => maxRemove(i)}
				...
				{#if maxTags.length < MAX_TAGS}
					<input
						...
						placeholder="Add tag…"
						style="flex: 1; min-width: 80px; border: none; background: transparent; font-size: 13px; color: var(--dt2); outline: none; padding: 2px 0;"
						aria-label="New tag"
					/>
				{:else}
					<span style="font-size: 12px; color: var(--dt4); padding: 2px 4px;">Max {MAX_TAGS} reached</span>
				{/if}
			</div>
			<div style="margin-top: 6px;">
				<div style="display: flex; gap: 3px;">
					{#each Array(MAX_TAGS) as _, i}
						<div style="height: 2px; flex: 1; border-radius: 1px; background: {i < maxTags.length ? 'var(--dt)' : 'var(--dbd)'}; transition: background 0.2s;"></div>
					{/each}
				</div>
				<div style="font-size: 11px; color: var(--dt4); margin-top: 4px;">{maxTags.length} / {MAX_TAGS} tags used</div>
			</div>
		</div>
	</div>
	...
	<div class="ds-card">
		...
		<div style="max-width: 480px;">
			<div style="
				...
				padding: 8px 10px; border-radius: 8px;
				background: var(--dbg);
				border: 1px solid {validError ? 'rgba(239,68,68,0.35)' : 'var(--dbd)'};
			...
			{:else}
				<div style="margin-top: 6px; font-size: 11px; color: var(--dt4);">{validTags.length} tag{validTags.length !== 1 ? 's' : ''}</div>
			{/if}
		</div>
	</div>
	...
	<div class="ds-card">
		...
		<div style="display: flex; flex-wrap: wrap; gap: 6px; max-width: 480px;">
			{#each ['Svelte 5', 'TypeScript', 'SvelteKit 2', 'Tailwind', 'PostgreSQL', 'Redis', 'Docker'] as tag}
				<span style="
					...
					background: var(--dbg2); border: 1px solid var(--dbd);
					font-size: 12px; font-weight: 500; color: var(--dt3);
				">{tag}</span>
			{/each}
		</div>
		...
		<div style="display: flex; flex-wrap: wrap; gap: 6px; max-width: 480px;">
			{#each ['Frontend', 'Backend', 'DevOps', 'Security', 'QA'] as tag}
				<span style="
					...
					background: transparent; border: 1px solid var(--dbd);
		...
		<div style="max-width: 360px; padding: 16px; border-radius: 12px; background: var(--dbg); border: 1px solid var(--dbd);">
			<div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 8px;">
				<h5 style="margin: 0; font-size: 14px; font-weight: 600; color: var(--dt);">Auth Refactor</h5>
				<span style="font-size: 10px; color: var(--dt4);">Mar 9</span>
			</div>
			<p style="margin: 0 0 12px; font-size: 12px; color: var(--dt2); line-height: 1.5;">Migrate session handling from cookies to JWTs with short-lived access tokens and refresh rotation.</p>
			<div style="display: flex; flex-wrap: wrap; gap: 5px;">
				...
				{#each ['Backend', 'Go'] as tag}
					<span style="display: inline-flex; align-items: center; padding: 2px 8px; border-radius: 9999px; background: var(--dbg2); border: 1px solid var(--dbd); font-size: 11px; color: var(--dt3);">{tag}</span>
				{/each}
			</div>
		</div>
	</div>
