...
<section class="ds-section">
	...
	<div class="ds-card">
		...
		<div class="ds-demo">
			<div role="group" aria-label="View mode" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each [
					{ val: 'list', icon: 'list', label: 'List' },
					...
					<button
						...
						style="display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; font-size: 13px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {viewMode === item.val ? 'var(--dt)' : 'transparent'}; color: {viewMode === item.val ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						<span style="font-size: 14px;">{#if item.icon === 'list'}<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>{:else}{item.icon}{/if}</span>
						{item.label}
					</button>
				{/each}
			</div>
			<span style="font-size: 12px; color: var(--dt2);">Selected: <strong style="color: var(--dt3);">{viewMode}</strong></span>
		</div>

		<h4 class="ds-card__h4">Time Range</h4>
		<div class="ds-demo">
			<div role="group" aria-label="Time range" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each ['day', 'week', 'month', 'year'] as range}
					<button
						onclick={() => timeRange = range as typeof timeRange}
						aria-pressed={timeRange === range}
						style="padding: 6px 14px; font-size: 13px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {timeRange === range ? 'var(--dt)' : 'transparent'}; color: {timeRange === range ? 'var(--dbg)' : 'var(--dt2)'}; text-transform: capitalize;"
					>
						{range}
					</button>
				{/each}
	...
	<div class="ds-card">
		...
		<div class="ds-demo" style="gap: 16px; align-items: center;">
			<div role="group" aria-label="Font style" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each [
					...
					<button
						...
						style="display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 32px; font-size: 14px; font-family: serif; {item.style} border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {fontStyles.has(item.val) ? 'var(--dbg2)' : 'transparent'}; color: {fontStyles.has(item.val) ? 'var(--dt)' : 'var(--dt2)'}; outline: {fontStyles.has(item.val) ? '1px solid var(--dbd2)' : 'none'}; outline-offset: 0;"
					>
						{item.icon}
					</button>
				{/each}
			</div>
			<div style="font-size: 14px; color: var(--dt3); padding: 6px 12px; background: var(--dbg); border-radius: 6px; {fontStyles.has('bold') ? 'font-weight: 700;' : ''} {fontStyles.has('italic') ? 'font-style: italic;' : ''} {fontStyles.has('underline') ? 'text-decoration: underline;' : ''} {fontStyles.has('strike') ? 'text-decoration: line-through;' : ''}">
				Sample text
			</div>
		</div>
			...
			<div role="group" aria-label="Category filters" style="display: inline-flex; flex-wrap: wrap; gap: 4px;">
				{#each ['frontend', 'backend', 'database', 'devops', 'security', 'design'] as filter}
					<button
						...
						style="padding: 5px 12px; font-size: 12px; font-weight: 500; border: 1px solid {activeFilters.has(filter) ? 'var(--dbd2)' : 'var(--dbd)'}; border-radius: 9999px; cursor: pointer; transition: background 0.12s, color 0.12s, border-color 0.12s; background: {activeFilters.has(filter) ? 'var(--dbg2)' : 'transparent'}; color: {activeFilters.has(filter) ? 'var(--dt)' : 'var(--dt2)'}; text-transform: capitalize;"
					>
						{filter}
					</button>
				{/each}
		...
		{#if activeFilters.size > 0}
			<p style="margin: 8px 0 0; font-size: 12px; color: var(--dt2);">
	...
	<div class="ds-card">
		...
		<div class="ds-demo">
			<div role="group" aria-label="Small toggle group" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 7px; padding: 2px; gap: 2px;">
				{#each ['A', 'B', 'C'] as opt}
					<button
						onclick={() => sizeSm = opt}
						aria-pressed={sizeSm === opt}
						style="padding: 3px 10px; font-size: 12px; font-weight: 500; border: none; border-radius: 5px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {sizeSm === opt ? 'var(--dt)' : 'transparent'}; color: {sizeSm === opt ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						Option {opt}
					</button>
				{/each}
			</div>
			<span style="font-size: 11px; color: var(--dt2);">sm — 26px height</span>
		</div>

		<h4 class="ds-card__h4">Default</h4>
		<div class="ds-demo">
			<div role="group" aria-label="Default toggle group" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each ['A', 'B', 'C'] as opt}
					<button
						onclick={() => sizeDef = opt}
						aria-pressed={sizeDef === opt}
						style="padding: 6px 14px; font-size: 13px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {sizeDef === opt ? 'var(--dt)' : 'transparent'}; color: {sizeDef === opt ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						Option {opt}
					</button>
				{/each}
			</div>
			<span style="font-size: 11px; color: var(--dt2);">default — 32px height</span>
		</div>

		<h4 class="ds-card__h4">Large</h4>
		<div class="ds-demo">
			<div role="group" aria-label="Large toggle group" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 11px; padding: 4px; gap: 2px;">
				{#each ['A', 'B', 'C'] as opt}
					<button
						onclick={() => sizeLg = opt}
						aria-pressed={sizeLg === opt}
						style="padding: 9px 18px; font-size: 14px; font-weight: 500; border: none; border-radius: 8px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {sizeLg === opt ? 'var(--dt)' : 'transparent'}; color: {sizeLg === opt ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						Option {opt}
					</button>
				{/each}
			</div>
			<span style="font-size: 11px; color: var(--dt2);">lg — 40px height</span>
		</div>
	</div>
	...
	<div class="ds-card">
			...
			<div role="group" aria-label="Outline variant" style="display: inline-flex; gap: 4px;">
				{#each ['Left', 'Center', 'Right'] as opt}
					<button
						...
						style="padding: 6px 14px; font-size: 13px; font-weight: 500; border: 1px solid {variantOutline === opt ? 'var(--dbd2)' : 'var(--dbd)'}; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s, border-color 0.12s; background: {variantOutline === opt ? 'var(--dbg2)' : 'transparent'}; color: {variantOutline === opt ? 'var(--dt)' : 'var(--dt2)'};"
					>
						{opt}
					</button>
				{/each}
		...
		<div class="ds-demo">
			<div role="group" aria-label="Filled variant" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each ['Left', 'Center', 'Right'] as opt}
					<button
						onclick={() => variantFilled = opt}
						aria-pressed={variantFilled === opt}
						style="padding: 6px 14px; font-size: 13px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {variantFilled === opt ? 'var(--dt)' : 'transparent'}; color: {variantFilled === opt ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						{opt}
					</button>
				{/each}
			...
			<div role="group" aria-label="Ghost variant" style="display: inline-flex; gap: 2px;">
				{#each ['Left', 'Center', 'Right'] as opt}
					<button
						...
						style="padding: 6px 14px; font-size: 13px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {variantGhost === opt ? 'var(--dbg2)' : 'transparent'}; color: {variantGhost === opt ? 'var(--dt2)' : 'var(--dt2)'};"
					>
						{opt}
					</button>
				{/each}
	...
	<div class="ds-card">
		...
		<div class="ds-demo">
			<div role="group" aria-label="Text alignment" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each [
					...
					<button
						...
						style="display: inline-flex; align-items: center; gap: 5px; padding: 6px 10px; font-size: 12px; font-weight: 500; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {textAlign === item.val ? 'var(--dt)' : 'transparent'}; color: {textAlign === item.val ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						{@html item.svg}
						{item.label}
					</button>
		...
		<div class="ds-demo">
			<div role="group" aria-label="View mode icon toggle" style="display: inline-flex; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each [
					...
					<button
						...
						style="display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 32px; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {viewMode === item.val ? 'var(--dt)' : 'transparent'}; color: {viewMode === item.val ? 'var(--dbg)' : 'var(--dt2)'};"
					>
						{@html item.svg}
					</button>
				{/each}
	...
	<div class="ds-card">
		...
		<div class="ds-demo" style="align-items: flex-start; gap: 32px;">
			<div role="group" aria-label="Vertical toggle" style="display: inline-flex; flex-direction: column; background: var(--dbg); border: 1px solid var(--dbd); border-radius: 9px; padding: 3px; gap: 2px;">
				{#each ['Option 1', 'Option 2', 'Option 3', 'Option 4'] as opt, i}
					<button
						onclick={() => verticalSel = `option${i + 1}`}
						aria-pressed={verticalSel === `option${i + 1}`}
						style="padding: 7px 20px; font-size: 13px; font-weight: 500; text-align: left; border: none; border-radius: 6px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {verticalSel === `option${i + 1}` ? 'var(--dt)' : 'transparent'}; color: {verticalSel === `option${i + 1}` ? 'var(--dbg)' : 'var(--dt2)'}; white-space: nowrap;"
					>
						{opt}
					</button>
				{/each}
			...
			<div role="group" aria-label="Vertical nav toggle" style="display: inline-flex; flex-direction: column; gap: 2px; width: 160px;">
				{#each [
					...
					<button
						...
						style="display: flex; align-items: center; gap: 8px; padding: 8px 12px; font-size: 13px; font-weight: 500; text-align: left; border: none; border-radius: 7px; cursor: pointer; transition: background 0.12s, color 0.12s; background: {verticalSel === item.val ? 'var(--dbg2)' : 'transparent'}; color: {verticalSel === item.val ? 'var(--dt)' : 'var(--dt2)'}; width: 100%;"
					>
						{@html item.svg}
						{item.label}
					</button>
