<script lang="ts">
	...
	const thStyle = 'text-align: left; padding: 11px 16px; color: var(--dt2); font-weight: 500; font-size: 11px; text-transform: uppercase; letter-spacing: 0.06em; white-space: nowrap;';
	const tdStyle = 'padding: 12px 16px; color: var(--dt); font-size: 13px;';
	const tdStyleCompact = 'padding: 7px 12px; color: var(--dt); font-size: 12px;';
	const thStyleCompact = 'text-align: left; padding: 7px 12px; color: var(--dt2); font-weight: 500; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em;';
	const rowBorder = 'border-bottom: 1px solid var(--dbd);';
	const tableWrap = 'border: 1px solid var(--dbd); border-radius: 10px; overflow: hidden; background: var(--dbg);';
</script>
		...
		<div class="ds-demo" style="display: block;">
			<div style={tableWrap}>
				<table style="width: 100%; border-collapse: collapse; font-size: 13px;">
					<thead>
						<tr style="border-bottom: 1px solid var(--dbd); background: var(--dbg);">
					...
					<tbody>
						{#each users as user, i}
							<tr style="{i < users.length - 1 ? rowBorder : ''}">
								<td style="{tdStyle} color: var(--dt); font-weight: 500;">{user.name}</td>
								<td style="{tdStyle} color: var(--dt3);">{user.email}</td>
								<td style={tdStyle}>{user.role}</td>
								<td style="{tdStyle} color: var(--dt3);">{user.joined}</td>
				...
				<table style="width: 100%; border-collapse: collapse; font-size: 13px;">
					...
					<tbody>
						{#each projects as project, i}
							...
							<tr style="background: {i % 2 === 1 ? 'var(--dbg)' : 'transparent'};">
								<td style="{tdStyle} color: var(--dt); font-weight: 500;">{project.name}</td>
								...
								<td style={tdStyle}>
									<div style="display: flex; align-items: center; gap: 8px;">
										<div style="flex: 1; height: 4px; background: var(--dbd); border-radius: 9999px; min-width: 80px;">
											<div style="height: 4px; background: {project.progress === 100 ? '#22c55e' : 'var(--dt4)'}; border-radius: 9999px; width: {project.progress}%;"></div>
										</div>
										<span style="font-size: 12px; color: var(--dt3); width: 28px; text-align: right;">{project.progress}%</span>
									</div>
								</td>
								<td style="{tdStyle} color: var(--dt3);">{project.due}</td>
							</tr>
						{/each}
					</tbody>
				</table>
					...
					<tbody>
						{#each users as user, i}
							...
							<tr
								...
								onmouseenter={(e) => (e.currentTarget.style.background = 'var(--dbg2)')}
								onmouseleave={(e) => (e.currentTarget.style.background = 'transparent')}
							>
								<td style="{tdStyle} color: var(--dt); font-weight: 500;">
									<div style="display: flex; align-items: center; gap: 10px;">
										<div style="width: 30px; height: 30px; border-radius: 50%; background: var(--dbg2); display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: var(--dt3); flex-shrink: 0;">
											{user.name.charAt(0)}
										</div>
										<div>
											<div style="font-size: 13px; font-weight: 500; color: var(--dt);">{user.name}</div>
											<div style="font-size: 11px; color: var(--dt2);">{user.email}</div>
										</div>
									</div>
								</td>
								<td style={tdStyle}>
								...
								<td style={tdStyle}>{user.role}</td>
								<td style="{tdStyle} color: var(--dt2);">{user.lastActive}</td>
							</tr>
						{/each}
					</tbody>
				</table>
			...
			<div style={tableWrap}>
				<table style="width: 100%; border-collapse: collapse; font-size: 12px;">
					...
					<tbody>
						{#each users as user, i}
							{@const sc = statusColors[user.status]}
							<tr style="{i < users.length - 1 ? 'border-bottom: 1px solid var(--dbd);' : ''}">
								<td style="{tdStyleCompact} color: var(--dt); font-weight: 500;">{user.name}</td>
								<td style="{tdStyleCompact} color: var(--dt3);">{user.role}</td>
								<td style="{tdStyleCompact} color: var(--dt3);">{user.projects}</td>
								...
								<td style="{tdStyleCompact} color: var(--dt2);">{user.joined}</td>
							</tr>
						{/each}
					</tbody>
				</table>
								...
								<td style="{tdStyle} text-align: right;">
									<div style="display: flex; align-items: center; justify-content: flex-end; gap: 4px;">
										<button
											...
											style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--dbd); background: transparent; color: var(--dt3); cursor: pointer; transition: background 0.1s, color 0.1s;"
											onmouseenter={(e) => { (e.currentTarget as HTMLElement).style.background = 'var(--dbg2)'; (e.currentTarget as HTMLElement).style.color = 'var(--dt)'; }}
											onmouseleave={(e) => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = 'var(--dt3)'; }}
										>
										...
										<button
											...
											onmouseleave={(e) => { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = 'var(--dt3)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--dbd)'; }}
										>
					...
					<thead>
						<tr style="border-bottom: 1px solid var(--dbd); background: var(--dbg);">
							{#each [['name','Name'],['role','Role'],['projects','Projects'],['joined','Joined'],['lastActive','Last Active']] as [col, label]}
								...
								<th style="{thStyle} cursor: pointer; user-select: none; color: {isActive ? 'var(--dt)' : 'var(--dt2)'}; transition: color 0.15s;">
									<button
										onclick={() => setSort(col)}
					...
					<tbody>
						{#each sortedUsers as user, i}
							<tr style="{i < sortedUsers.length - 1 ? rowBorder : ''}">
								...
								<td style={tdStyle}>{user.projects}</td>
								<td style="{tdStyle} color: var(--dt3);">{user.joined}</td>
								<td style="{tdStyle} color: var(--dt3);">{user.lastActive}</td>
							</tr>
						{/each}
					</tbody>
				</table>
						...
						<tr>
							<td colspan={4} style="padding: 48px 16px; text-align: center;">
								<div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
									<div style="width: 40px; height: 40px; border-radius: 10px; background: var(--dbg); display: flex; align-items: center; justify-content: center;">
										<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="color: var(--dt2);"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
									</div>
									<div>
										<p style="margin: 0; font-size: 14px; font-weight: 500; color: var(--dt);">No members found</p>
										<p style="margin: 4px 0 0; font-size: 13px; color: var(--dt2);">Get started by inviting your first team member.</p>
									</div>
									<button style="margin-top: 4px; padding: 7px 14px; border-radius: 8px; border: 1px solid var(--dbd2); background: var(--dbg2); color: var(--dt); font-size: 13px; font-weight: 500; cursor: pointer;">
										Invite member
									</button>
								</div>
							</td>
