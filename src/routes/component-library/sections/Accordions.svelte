...
<section class="ds-section">
		...
		<div class="ds-demo" style="display: block;">
			<div style="display: flex; flex-direction: column; gap: 4px; max-width: 600px;">
				{#each basicItems as item, i}
					<div style="border: 1px solid rgba(255,255,255,0.06); border-radius: 8px; overflow: hidden; background: rgba(255,255,255,0.01);">
						<button
							onclick={() => basicToggle(i)}
							aria-expanded={basicOpen === i}
							style="width: 100%; padding: 14px 16px; display: flex; justify-content: space-between; align-items: center; background: {basicOpen === i ? 'rgba(255,255,255,0.04)' : 'transparent'}; border: none; color: {basicOpen === i ? '#fff' : '#ccc'}; font-size: 14px; font-weight: {basicOpen === i ? '500' : '400'}; cursor: pointer; text-align: left; transition: background 0.15s, color 0.15s;"
						>
						...
						{#if basicOpen === i}
							<div style="padding: 0 16px 14px; font-size: 13px; color: #999; line-height: 1.6; border-top: 1px solid rgba(255,255,255,0.04);">
								{item.body}
							</div>
						{/if}
					</div>
		...
		<div class="ds-demo" style="display: block;">
			<div style="display: flex; flex-direction: column; gap: 4px; max-width: 600px;">
				{#each iconItems as item, i}
					...
					<div style="border: 1px solid rgba(255,255,255,0.06); border-radius: 10px; overflow: hidden; background: rgba(255,255,255,0.01);">
						<button
							onclick={() => iconToggle(i)}
							aria-expanded={isOpen}
							style="width: 100%; padding: 14px 16px; display: flex; align-items: center; gap: 12px; background: {isOpen ? 'rgba(255,255,255,0.04)' : 'transparent'}; border: none; color: #fff; font-size: 14px; cursor: pointer; text-align: left; transition: background 0.15s;"
						>
							<!-- Leading icon -->
							<div style="flex-shrink: 0; width: 28px; height: 28px; border-radius: 7px; background: rgba(255,255,255,0.06); display: flex; align-items: center; justify-content: center; color: #aaa;">
								{#if i === 0}
							...
							<svg
								...
								stroke="currentColor"
								stroke-width="2"
								style="flex-shrink: 0; color: #666; transform: rotate({isOpen ? '180deg' : '0deg'}); transition: transform 0.2s ease;"
							>
								<path d="M6 9l6 6 6-6" />
							</svg>
						</button>
						{#if isOpen}
							<div style="padding: 12px 16px 14px 56px; font-size: 13px; color: #999; line-height: 1.6; border-top: 1px solid rgba(255,255,255,0.04);">
								{item.body}
							</div>
						{/if}
					</div>
	...
	<div class="ds-card">
		...
		<div class="ds-demo" style="display: block;">
			<div style="max-width: 600px; border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; overflow: hidden;">
				{#each borderedItems as item, i}
					<div style="{i > 0 ? 'border-top: 1px solid rgba(255,255,255,0.08);' : ''}">
						<button
							onclick={() => borderedToggle(i)}
							aria-expanded={borderedOpen === i}
							style="width: 100%; padding: 15px 18px; display: flex; justify-content: space-between; align-items: center; background: {borderedOpen === i ? 'rgba(255,255,255,0.03)' : 'transparent'}; border: none; color: #ddd; font-size: 14px; font-weight: 500; cursor: pointer; text-align: left; transition: background 0.15s;"
						>
							<span>{item.title}</span>
						...
						{#if borderedOpen === i}
							<div style="padding: 0 18px 15px; font-size: 13px; color: #888; line-height: 1.65; border-top: 1px solid rgba(255,255,255,0.05);">
				...
				{#each flushItems as item, i}
					<div style="{i > 0 ? 'border-top: 1px solid rgba(255,255,255,0.05);' : ''}">
						<button
							...
							style="width: 100%; padding: 14px 0; display: flex; justify-content: space-between; align-items: center; background: transparent; border: none; color: {flushOpen === i ? '#fff' : '#bbb'}; font-size: 14px; cursor: pointer; text-align: left; transition: color 0.15s;"
						>
							<span>{item.title}</span>
							<svg
								width="16"
								...
								stroke="currentColor"
								stroke-width="2"
								style="flex-shrink: 0; color: #555; transform: rotate({flushOpen === i ? '180deg' : '0deg'}); transition: transform 0.2s ease;"
							>
								<path d="M6 9l6 6 6-6" />
							</svg>
						</button>
						{#if flushOpen === i}
							<div style="padding-bottom: 14px; font-size: 13px; color: #888; line-height: 1.65;">
								{item.body}
							</div>
						{/if}
					</div>
		...
		<div class="ds-demo" style="display: block;">
			<div style="display: flex; flex-direction: column; gap: 6px; max-width: 640px;">
				{#each faqItems as item, i}
					...
					<div style="border-radius: 10px; overflow: hidden; border: 1px solid {isOpen ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)'}; background: {isOpen ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.01)'}; transition: border-color 0.15s, background 0.15s;">
						<button
							onclick={() => faqToggle(i)}
							aria-expanded={isOpen}
							style="width: 100%; padding: 14px 16px; display: flex; align-items: flex-start; gap: 12px; background: transparent; border: none; color: #ddd; font-size: 14px; cursor: pointer; text-align: left;"
						>
							<span style="flex-shrink: 0; width: 22px; height: 22px; border-radius: 50%; background: rgba(255,255,255,0.06); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: #888; margin-top: 1px;">{i + 1}</span>
							<span style="flex: 1; font-weight: {isOpen ? '500' : '400'}; color: {isOpen ? '#fff' : '#ccc'};">{item.q}</span>
							<svg
								width="16"
								height="16"
								viewBox="0 0 24 24"
								...
								stroke="currentColor"
								stroke-width="2"
								style="flex-shrink: 0; color: #555; margin-top: 3px; transform: rotate({isOpen ? '180deg' : '0deg'}); transition: transform 0.2s ease;"
							>
								<path d="M6 9l6 6 6-6" />
							</svg>
						</button>
						{#if isOpen}
							<div style="padding: 0 16px 14px 50px; font-size: 13px; color: #999; line-height: 1.7; border-top: 1px solid rgba(255,255,255,0.05);">
	...
	<div class="ds-card">
		...
		<div class="ds-demo" style="display: block;">
			<div style="max-width: 480px; border: 1px solid rgba(255,255,255,0.07); border-radius: 12px; overflow: hidden; background: rgba(255,255,255,0.01);">
				<!-- Panel header -->
				<div style="padding: 14px 16px; border-bottom: 1px solid rgba(255,255,255,0.06); display: flex; align-items: center; justify-content: space-between;">
					<span style="font-size: 13px; font-weight: 600; color: #fff;">Settings</span>
					<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: #555;"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z" stroke-linecap="round" stroke-linejoin="round"/></svg>
				</div>
				{#each [
					...
					<div style="{gi > 0 ? 'border-top: 1px solid rgba(255,255,255,0.05);' : ''}">
						<button
							onclick={() => { flushOpen = isGroupOpen ? -1 : 10 + gi; }}
							aria-expanded={isGroupOpen}
							style="width: 100%; padding: 12px 16px; display: flex; align-items: center; gap: 10px; background: {isGroupOpen ? 'rgba(255,255,255,0.03)' : 'transparent'}; border: none; color: #ccc; font-size: 13px; font-weight: 500; cursor: pointer; text-align: left; transition: background 0.15s;"
						>
							<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: #666; flex-shrink: 0;">
							...
							<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: #555; transform: rotate({isGroupOpen ? '180deg' : '0deg'}); transition: transform 0.2s ease;"><path d="M6 9l6 6 6-6"/></svg>
						</button>
						{#if isGroupOpen}
							<div style="border-top: 1px solid rgba(255,255,255,0.04);">
								{#each group.items as subItem}
									<button style="width: 100%; padding: 9px 16px 9px 40px; display: flex; align-items: center; gap: 8px; background: transparent; border: none; color: #888; font-size: 13px; cursor: pointer; text-align: left; transition: color 0.12s;" onmouseenter={(e) => (e.currentTarget.style.color = '#ccc')} onmouseleave={(e) => (e.currentTarget.style.color = '#888')}>
