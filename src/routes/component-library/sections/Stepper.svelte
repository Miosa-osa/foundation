...
<section class="ds-section">
				...
				{#each hSteps as step, i}
					...
					<div style="display: flex; flex-direction: column; align-items: center; gap: 8px; flex-shrink: 0;">
						...
						<div style="
							...
							background: {done ? '#fff' : active ? 'rgba(255,255,255,0.08)' : 'transparent'};
							border: 2px solid {done ? '#fff' : active ? '#fff' : 'rgba(255,255,255,0.15)'};
							color: {done ? '#000' : active ? '#fff' : '#444'};
						...
						<!-- Label -->
						<span style="font-size: 11px; font-weight: {active ? 600 : 400}; color: {done ? '#888' : active ? '#fff' : '#444'}; white-space: nowrap;">{step.label}</span>
					</div>
					<!-- Connector -->
					{#if i < hSteps.length - 1}
						<div style="flex: 1; height: 2px; background: {done ? '#fff' : 'rgba(255,255,255,0.08)'}; margin: 0 6px; margin-bottom: 28px; min-width: 24px; transition: background 0.3s;"></div>
					{/if}
				{/each}
			</div>
		</div>
	...
	<div class="ds-card">
		...
		<div style="max-width: 560px;">
				...
				{#each hSteps as step, i}
					...
					<div style="display: flex; flex-direction: column; align-items: center; gap: 8px; flex-shrink: 0;">
						<div style="
							width: 32px; height: 32px; border-radius: 50%;
							background: {done ? '#fff' : active ? 'rgba(255,255,255,0.08)' : 'transparent'};
							border: 2px solid {done ? '#fff' : active ? '#fff' : 'rgba(255,255,255,0.12)'};
							color: {done ? '#000' : active ? '#fff' : '#444'};
							display: flex; align-items: center; justify-content: center;
						...
						<div style="text-align: center;">
							<div style="font-size: 11px; font-weight: {active ? 600 : 400}; color: {done ? '#777' : active ? '#fff' : '#444'}; white-space: nowrap;">{step.label}</div>
							<div style="font-size: 10px; color: #444; white-space: nowrap;">{step.desc}</div>
						</div>
					</div>
					{#if i < hSteps.length - 1}
						<div style="flex: 1; height: 2px; background: {done ? '#fff' : 'rgba(255,255,255,0.07)'}; margin: 0 6px; margin-bottom: 28px; transition: background 0.25s;"></div>
					{/if}
				{/each}
			</div>
			<!-- Controls -->
			<div style="display: flex; align-items: center; gap: 10px;">
				...
				<span style="font-size: 12px; color: #444; margin-left: 4px;">Step {hStep} of {hSteps.length}</span>
			</div>
		</div>
	</div>
			...
			{#each vSteps as step, i}
				...
				<div style="display: flex; gap: 14px; position: relative;">
					...
					<div style="display: flex; flex-direction: column; align-items: center; flex-shrink: 0; width: 32px;">
						<div style="
							width: 32px; height: 32px; border-radius: 50%;
							background: {done ? '#fff' : active ? 'rgba(255,255,255,0.06)' : 'transparent'};
							border: 2px solid {done ? '#fff' : active ? '#fff' : 'rgba(255,255,255,0.12)'};
							color: {done ? '#000' : active ? '#fff' : '#444'};
							display: flex; align-items: center; justify-content: center;
						...
						{#if i < vSteps.length - 1}
							<div style="width: 2px; flex: 1; min-height: 12px; background: {done ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.06)'}; margin-top: 4px; transition: background 0.25s;"></div>
						{/if}
					</div>
					<!-- Content -->
					<div style="flex: 1; padding-bottom: 24px; padding-top: 4px;">
						<div style="font-size: 13px; font-weight: 600; color: {done ? '#666' : active ? '#fff' : '#444'}; margin-bottom: 4px;">{step.label}</div>
						<div style="font-size: 12px; color: {active ? '#888' : '#444'}; line-height: 1.5; max-width: 360px;">{step.desc}</div>
						{#if active}
	...
	<div class="ds-card">
		...
		<div style="max-width: 520px; border: 1px solid rgba(255,255,255,0.07); border-radius: 12px; overflow: hidden;">
			<!-- Step bar -->
			<div style="padding: 20px 24px; border-bottom: 1px solid rgba(255,255,255,0.06); display: flex; align-items: center; gap: 0; background: rgba(255,255,255,0.01);">
					...
					<div style="display: flex; align-items: center; flex: {i < wizardSteps.length - 1 ? '1' : '0'};">
						<div style="display: flex; align-items: center; gap: 8px; flex-shrink: 0;">
							<div style="
								...
								background: {done ? '#fff' : active ? 'rgba(255,255,255,0.1)' : 'transparent'};
								border: 1.5px solid {done ? '#fff' : active ? '#fff' : 'rgba(255,255,255,0.12)'};
								color: {done ? '#000' : active ? '#fff' : '#444'};
								display: flex; align-items: center; justify-content: center;
							...
							<span style="font-size: 12px; font-weight: {active ? 600 : 400}; color: {done ? '#666' : active ? '#fff' : '#444'}; white-space: nowrap;">{label}</span>
						</div>
						{#if i < wizardSteps.length - 1}
							<div style="flex: 1; height: 1px; background: {done ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.06)'}; margin: 0 10px; transition: background 0.25s;"></div>
						{/if}
					</div>
				{/each}
			</div>
			...
			<div style="padding: 28px 24px;">
				<h4 style="margin: 0 0 10px; font-size: 16px; font-weight: 600; color: #fff;">{wizardContent[wizardStep].heading}</h4>
				<p style="margin: 0 0 24px; font-size: 13px; color: #777; line-height: 1.6;">{wizardContent[wizardStep].body}</p>
				...
				<div style="display: flex; gap: 14px; position: relative;">
					<div style="display: flex; flex-direction: column; align-items: center; flex-shrink: 0; width: 36px;">
						<div style="
							...
							background: {done ? '#fff' : active ? 'rgba(255,255,255,0.07)' : 'transparent'};
							border: 1.5px solid {done ? '#fff' : active ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.1)'};
							color: {done ? '#000' : active ? '#fff' : '#444'};
						...
						{#if i < 3}
							<div style="width: 2px; flex: 1; min-height: 12px; background: {done ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)'}; margin-top: 4px;"></div>
						{/if}
					</div>
					<div style="flex: 1; padding-bottom: 20px; padding-top: 7px;">
						<span style="font-size: 13px; font-weight: {active ? 600 : 400}; color: {done ? '#777' : active ? '#fff' : '#444'};">{step.label}</span>
					</div>
				</div>
			{/each}
		</div>
			...
			{#each [2, 3, 4, 1] as active, ri}
				...
				<div style="display: flex; align-items: center; gap: 0;">
					{#each Array(total) as _, i}
						...
						<div style="width: {isCurrent ? '24px' : '8px'}; height: 8px; border-radius: 4px; background: {done ? '#fff' : isCurrent ? '#fff' : 'rgba(255,255,255,0.1)'}; transition: all 0.25s; flex-shrink: 0; {i < total - 1 ? 'margin-right: 5px;' : ''}"></div>
					{/each}
					<span style="font-size: 11px; color: #444; margin-left: 12px;">{active} / {total}</span>
				</div>
			{/each}
		</div>
	</div>
			...
			<div style="display: flex; align-items: center; margin-bottom: 16px;">
				{#each errorSteps as label, i}
					...
					<div style="display: flex; flex-direction: column; align-items: center; gap: 8px; flex-shrink: 0;">
						<div style="
							width: 32px; height: 32px; border-radius: 50%;
							background: {isError ? 'rgba(239,68,68,0.1)' : done ? '#fff' : active ? 'rgba(255,255,255,0.08)' : 'transparent'};
							border: 2px solid {isError ? '#ef4444' : done ? '#fff' : active ? '#fff' : 'rgba(255,255,255,0.12)'};
							color: {isError ? '#ef4444' : done ? '#000' : active ? '#fff' : '#444'};
							display: flex; align-items: center; justify-content: center;
						...
						<span style="font-size: 11px; font-weight: {active || isError ? 600 : 400}; color: {isError ? '#ef4444' : done ? '#777' : active ? '#fff' : '#444'}; white-space: nowrap;">{label}</span>
					</div>
					{#if i < errorSteps.length - 1}
						<div style="flex: 1; height: 2px; background: {done && i + 1 !== hasError - 1 ? '#fff' : 'rgba(255,255,255,0.07)'}; margin: 0 6px; margin-bottom: 28px;"></div>
					{/if}
				{/each}
			</div>
			<!-- Error message -->
