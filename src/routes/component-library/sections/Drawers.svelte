		...
		<div class="ds-demo">
			<button class="btn-pill btn-pill-secondary btn-pill-sm" onclick={() => (filtersOpen = true)}>
				...
				{#if selectedStatuses.length < 3 || selectedPriority !== 'all'}
					<span style="background: rgba(255,255,255,0.15); border-radius: 9999px; padding: 1px 6px; font-size: 11px;">
		...
		>
			...
			<div class="drawer-body">
				<div style="display: flex; flex-direction: column; gap: 20px;">
					<div style="border-radius: 10px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); padding: 16px;">
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;">Overview</p>
						<div style="display: flex; flex-direction: column; gap: 10px;">
							{#each [['Project', 'MIOSA Platform'], ['Status', 'In progress'], ['Owner', 'R. Moreau'], ['Due date', 'Mar 14, 2026']] as [label, value]}
								<div style="display: flex; justify-content: space-between; align-items: center;">
									...
									<span style="font-size: 13px; color: #ddd; font-weight: 500;">{value}</span>
								</div>
							{/each}
						</div>
					</div>
					<div>
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;">Description</p>
					...
					<div>
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;">Activity</p>
						<div style="display: flex; flex-direction: column; gap: 12px;">
							{#each [
								...
								<div style="display: flex; align-items: center; gap: 10px;">
									<div style="width: 28px; height: 28px; border-radius: 9999px; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: #aaa; flex-shrink: 0;">{ev.avatar}</div>
									<div style="flex: 1; min-width: 0;">
										<span style="font-size: 13px; color: #ddd; font-weight: 500;">{ev.name}</span>
										<span style="font-size: 13px; color: #666;"> {ev.action}</span>
									</div>
									<span style="font-size: 12px; color: #444; flex-shrink: 0;">{ev.time}</span>
								</div>
							{/each}
						</div>
					</div>
					{#if rightSize === 'wide'}
						<div>
							<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;">Attachments</p>
							<div style="display: flex; flex-direction: column; gap: 8px;">
								{#each [['design-v3.fig', '2.4 MB', 'Figma'], ['spec.pdf', '840 KB', 'PDF'], ['api-schema.json', '12 KB', 'JSON']] as [name, size, type]}
									<div style="display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.06); background: rgba(255,255,255,0.02);">
										<div style="width: 32px; height: 32px; border-radius: 7px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: #666;">{type}</div>
										<div style="flex: 1; min-width: 0;">
											<p style="font-size: 13px; color: #ddd; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{name}</p>
											<p style="font-size: 11px; color: #555; margin: 1px 0 0;">{size}</p>
										</div>
										<button style="background: none; border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; color: #666; cursor: pointer; padding: 4px 8px; font-size: 12px;" aria-label="Download {name}">↓</button>
									</div>
								{/each}
							</div>
						</div>
...
{#if leftOpen}
	<div class="drawer-overlay drawer-overlay--left" onclick={() => (leftOpen = false)} role="dialog" aria-modal="true" aria-label="Navigation drawer">
		<div class="drawer-backdrop"></div>
		<div class="drawer-panel drawer-panel--left" style="width: 280px;" onclick={(e) => e.stopPropagation()} role="presentation">
			<div class="drawer-header">
				<div style="display: flex; align-items: center; gap: 10px;">
					<div style="width: 28px; height: 28px; border-radius: 7px; background: #fff; display: flex; align-items: center; justify-content: center;">
						<span style="font-size: 11px; font-weight: 900; color: #000;">M</span>
					</div>
					<span style="font-size: 14px; font-weight: 700; color: #fff; letter-spacing: -0.02em;">MIOSA</span>
				</div>
			...
			<div class="drawer-body" style="padding: 16px 12px;">
				<p style="font-size: 11px; font-weight: 600; color: #444; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 8px; padding: 0 8px;">Workspace</p>
				{#each [
					...
					<button
						style="width: 100%; display: flex; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 8px; border: none; background: {item.active ? 'rgba(255,255,255,0.08)' : 'transparent'}; color: {item.active ? '#fff' : '#888'}; font-size: 13px; font-weight: {item.active ? '500' : '400'}; cursor: pointer; text-align: left; margin-bottom: 2px; transition: background 0.12s, color 0.12s;"
				...
				{/each}
				<div style="margin: 16px 0 8px; border-top: 1px solid rgba(255,255,255,0.05);"></div>
				<p style="font-size: 11px; font-weight: 600; color: #444; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 8px; padding: 0 8px;">Settings</p>
				{#each [{ label: 'Account' }, { label: 'Billing' }, { label: 'Team' }] as item}
					<button style="width: 100%; display: flex; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 8px; border: none; background: transparent; color: #666; font-size: 13px; cursor: pointer; text-align: left; margin-bottom: 2px; transition: color 0.12s;" aria-label={item.label}>
						{item.label}
					</button>
				{/each}
			</div>
			<div class="drawer-footer" style="justify-content: flex-start;">
				<div style="display: flex; align-items: center; gap: 10px; flex: 1;">
					<div style="width: 30px; height: 30px; border-radius: 9999px; background: rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: #ccc;">R</div>
					<div>
						<p style="font-size: 13px; color: #ddd; font-weight: 500; margin: 0;">R. Moreau</p>
						<p style="font-size: 11px; color: #555; margin: 0;">Pro plan</p>
					</div>
				</div>
				<button class="drawer-close" onclick={() => (leftOpen = false)} aria-label="Close navigation">
...
<!-- ─── BOTTOM SHEET ───────────────────────────────────────────────────── -->
{#if bottomOpen}
	<div class="drawer-overlay drawer-overlay--bottom" onclick={() => (bottomOpen = false)} role="dialog" aria-modal="true" aria-label="Bottom sheet">
		<div class="drawer-backdrop"></div>
		<div class="drawer-panel drawer-panel--bottom" style="max-height: 60vh;" onclick={(e) => e.stopPropagation()} role="presentation">
			...
			<div style="padding: 12px 24px 0; display: flex; justify-content: center;">
				<div style="width: 36px; height: 4px; border-radius: 9999px; background: rgba(255,255,255,0.15);"></div>
			</div>
			...
			<div class="drawer-body" style="padding: 0 24px 8px;">
				<div style="display: flex; gap: 12px; margin-bottom: 20px;">
					{#each [
						...
						<button style="flex: 1; display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 14px 8px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.07); background: rgba(255,255,255,0.02); color: #aaa; cursor: pointer; font-size: 12px; transition: background 0.15s;" aria-label={action.label}>
							<div style="width: 36px; height: 36px; border-radius: 9px; background: rgba(255,255,255,0.07); display: flex; align-items: center; justify-content: center;">
				...
				<div style="border-top: 1px solid rgba(255,255,255,0.06); padding-top: 16px;">
					<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 10px;">People with access</p>
					{#each [
						...
						<div style="display: flex; align-items: center; gap: 10px; padding: 8px 0;">
							<div style="width: 32px; height: 32px; border-radius: 9999px; background: rgba(255,255,255,0.08); display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: #aaa; flex-shrink: 0;">{person.avatar}</div>
							<span style="flex: 1; font-size: 13px; color: #ccc;">{person.name}</span>
							<span style="font-size: 12px; color: #555;">{person.role}</span>
						</div>
					{/each}
				</div>
			</div>
...
<!-- ─── SETTINGS PANEL ─────────────────────────────────────────────────── -->
{#if settingsOpen}
	<div class="drawer-overlay drawer-overlay--right" onclick={() => (settingsOpen = false)} role="dialog" aria-modal="true" aria-label="Settings panel">
		<div class="drawer-backdrop"></div>
		<div class="drawer-panel drawer-panel--right" style="width: 480px;" onclick={(e) => e.stopPropagation()} role="presentation">
			...
			<!-- Tab bar -->
			<div style="display: flex; padding: 0 24px; border-bottom: 1px solid rgba(255,255,255,0.06); gap: 4px;">
				{#each [['general', 'General'], ['appearance', 'Appearance'], ['notifications', 'Notifications']] as [id, label]}
					<button
						...
						style="padding: 10px 12px; border: none; border-bottom: 2px solid {activeSettingsTab === id ? 'rgba(255,255,255,0.7)' : 'transparent'}; background: transparent; color: {activeSettingsTab === id ? '#fff' : '#666'}; font-size: 13px; font-weight: {activeSettingsTab === id ? '500' : '400'}; cursor: pointer; transition: color 0.15s; white-space: nowrap;"
						aria-label={label}
					>
						{label}
					</button>
			...
			<div class="drawer-body">
				{#if activeSettingsTab === 'general'}
					<div style="display: flex; flex-direction: column; gap: 20px;">
						{#each [['Display name', 'R. Moreau'], ['Email address', 'r.moreau@miosa.io'], ['Username', '@rmoreau']] as [label, value]}
							<div>
								<label style="display: block; font-size: 12px; font-weight: 500; color: #888; margin-bottom: 6px;">{label}</label>
								<input
									type="text"
									defaultValue={value}
									style="width: 100%; padding: 9px 12px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #ddd; font-size: 13px; outline: none; box-sizing: border-box;"
									aria-label={label}
								/>
							</div>
						{/each}
						<div>
							<label style="display: block; font-size: 12px; font-weight: 500; color: #888; margin-bottom: 6px;">Bio</label>
							<textarea
								style="width: 100%; padding: 9px 12px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #ddd; font-size: 13px; outline: none; resize: vertical; min-height: 80px; box-sizing: border-box; font-family: inherit;"
								aria-label="Bio"
				...
				{:else if activeSettingsTab === 'appearance'}
					<div style="display: flex; flex-direction: column; gap: 20px;">
						<div>
							<p style="font-size: 12px; font-weight: 500; color: #888; margin: 0 0 10px;">Theme</p>
							<div style="display: flex; gap: 8px;">
								{#each [['dark', 'Dark'], ['light', 'Light'], ['system', 'System']] as [id, label]}
									<button style="flex: 1; padding: 10px; border-radius: 8px; border: 1px solid {id === 'dark' ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.07)'}; background: {id === 'dark' ? 'rgba(255,255,255,0.06)' : 'transparent'}; color: {id === 'dark' ? '#fff' : '#666'}; font-size: 13px; cursor: pointer;" aria-label="Select {label} theme">{label}</button>
								{/each}
							</div>
						</div>
						<div>
							<p style="font-size: 12px; font-weight: 500; color: #888; margin: 0 0 10px;">Font size</p>
							<div style="display: flex; gap: 8px;">
								{#each [['sm', 'Small'], ['md', 'Medium'], ['lg', 'Large']] as [id, label]}
									<button style="flex: 1; padding: 10px; border-radius: 8px; border: 1px solid {id === 'md' ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.07)'}; background: {id === 'md' ? 'rgba(255,255,255,0.06)' : 'transparent'}; color: {id === 'md' ? '#fff' : '#666'}; font-size: 13px; cursor: pointer;" aria-label="Select {label} font size">{label}</button>
								{/each}
							</div>
						</div>
						<div style="display: flex; justify-content: space-between; align-items: center; padding: 14px 16px; border-radius: 10px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06);">
							<div>
								<p style="font-size: 13px; color: #ddd; font-weight: 500; margin: 0;">Reduced motion</p>
								<p style="font-size: 12px; color: #555; margin: 2px 0 0;">Minimise animations across the UI</p>
							</div>
							<div style="width: 38px; height: 22px; border-radius: 11px; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.1); cursor: pointer; position: relative;">
								<div style="position: absolute; top: 2px; left: 2px; width: 16px; height: 16px; border-radius: 9999px; background: #555;"></div>
							</div>
						</div>
					</div>
				{:else}
					<div style="display: flex; flex-direction: column; gap: 12px;">
						{#each [
							...
							<div style="display: flex; justify-content: space-between; align-items: center; padding: 14px 16px; border-radius: 10px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06);">
								<div>
									<p style="font-size: 13px; color: #ddd; font-weight: 500; margin: 0;">{item.label}</p>
									<p style="font-size: 12px; color: #555; margin: 2px 0 0;">{item.sub}</p>
								</div>
								<div style="width: 38px; height: 22px; border-radius: 11px; background: {item.on ? 'rgba(255,255,255,0.55)' : 'rgba(255,255,255,0.08)'}; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; position: relative; transition: background 0.2s;">
									<div style="position: absolute; top: 2px; {item.on ? 'right: 2px;' : 'left: 2px;'} width: 16px; height: 16px; border-radius: 9999px; background: {item.on ? '#111' : '#555'}; transition: right 0.2s, left 0.2s;"></div>
								</div>
							</div>
						{/each}
					</div>
...
<!-- ─── FILTERS PANEL ──────────────────────────────────────────────────── -->
{#if filtersOpen}
	<div class="drawer-overlay drawer-overlay--right" onclick={() => (filtersOpen = false)} role="dialog" aria-modal="true" aria-label="Filters panel">
		<div class="drawer-backdrop"></div>
		<div class="drawer-panel drawer-panel--right" style="width: 360px;" onclick={(e) => e.stopPropagation()} role="presentation">
			<div class="drawer-header">
				...
				<div style="display: flex; align-items: center; gap: 8px;">
					<button
						...
						style="font-size: 12px; color: #666; background: none; border: none; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: color 0.12s;"
			...
			<div class="drawer-body">
				<div style="display: flex; flex-direction: column; gap: 24px;">
					<div>
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 12px;">Status</p>
						<div style="display: flex; flex-direction: column; gap: 8px;">
							{#each [['active', 'Active', '#22c55e'], ['pending', 'Pending', '#f59e0b'], ['done', 'Done', '#6366f1']] as [id, label, color]}
								<label style="display: flex; align-items: center; gap: 10px; cursor: pointer; padding: 8px 10px; border-radius: 8px; border: 1px solid {selectedStatuses.includes(id) ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.04)'}; background: {selectedStatuses.includes(id) ? 'rgba(255,255,255,0.03)' : 'transparent'}; transition: all 0.15s;">
									<input
										type="checkbox"
									...
									<span style="font-size: 13px; color: {selectedStatuses.includes(id) ? '#ddd' : '#888'};">{label}</span>
								</label>
							{/each}
						</div>
					</div>
					<div>
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 12px;">Priority</p>
						<div style="display: flex; flex-direction: column; gap: 6px;">
							{#each [['all', 'All priorities'], ['high', 'High'], ['medium', 'Medium'], ['low', 'Low']] as [id, label]}
								<label style="display: flex; align-items: center; gap: 10px; cursor: pointer; padding: 8px 10px; border-radius: 8px; border: 1px solid {selectedPriority === id ? 'rgba(255,255,255,0.1)' : 'transparent'}; background: {selectedPriority === id ? 'rgba(255,255,255,0.03)' : 'transparent'}; transition: all 0.15s;">
									<input type="radio" name="priority" value={id} checked={selectedPriority === id} onchange={() => (selectedPriority = id)} style="accent-color: #fff;" aria-label={label} />
									<span style="font-size: 13px; color: {selectedPriority === id ? '#ddd' : '#888'};">{label}</span>
								</label>
							{/each}
						</div>
					</div>
					<div>
						<p style="font-size: 11px; font-weight: 600; color: #555; text-transform: uppercase; letter-spacing: 0.06em; margin: 0 0 12px;">Date range</p>
						<div style="display: flex; gap: 8px;">
							<div style="flex: 1;">
								<label style="display: block; font-size: 12px; color: #666; margin-bottom: 4px;">From</label>
								<input type="date" style="width: 100%; padding: 8px 10px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #ddd; font-size: 13px; outline: none; box-sizing: border-box; color-scheme: dark;" aria-label="Start date" />
							</div>
							<div style="flex: 1;">
								<label style="display: block; font-size: 12px; color: #666; margin-bottom: 4px;">To</label>
								<input type="date" style="width: 100%; padding: 8px 10px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #ddd; font-size: 13px; outline: none; box-sizing: border-box; color-scheme: dark;" aria-label="End date" />
							</div>
						</div>
					</div>
				</div>
